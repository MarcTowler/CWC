...
$(function()
{
    ...
    $('#files-nav li a').live('click', function(e) {
        ...
        if ($(this).attr('title') != 'upload')
        {
            $('#files_right_pane').load(href_val + ' #files-wrapper', function() {
                ...
                $('#files-wrapper').css('height', $(window).innerHeight() - 60);
            });
        }
        else
        {
    ...
    $( '#directory-box span.close, #directory-box a.cancel' ).on('click', function(e) {
        e.preventDefault();
        $( '#directory-box' ).fadeOut( 200, function() {
            $(this).find('input[type=text], input[type=file]').val('');
        });
    ...
    $('select[name=parent_id]').live('change', function() {
        ...
        var href_val = SITE_URL + 'admin/store/ajax/' + controller + '/' + folder_id;
