<div id="upload-box">
    ...
    <?php echo form_open_multipart('admin/store/ajax/upload'); ?>
...
<div id="directory-box" style="display: none;">
    <h2><?php echo lang('files:folders'); ?><span class="close ui-icon ui-icon-closethick"><?php echo lang('buttons.close'); ?></span></h2>
    <?php echo form_open_multipart('admin/store/ajax/subfolder'); ?>
    <?php echo form_hidden('redirect_to', 'image'); ?>
    <ul>
        
        <li>
            <label for="folder_id"><?php echo lang('files:parent'); ?>&nbsp;</label>
            <?php echo form_dropdown('folder_id', array(0 => lang('files:select_folder')) + $folders_tree, 'id="folder"'); ?>
        </li>
        <li>
            <label for="name"><?php echo lang('files:name'); ?></label>
            ...
            <?php foreach ($folders as $folder): ?>
                <?php if (!$folder->parent_id): ?>
                    <li id="folder-id-<?php echo $folder->id; ?>" class="<?php echo $current_folder && $current_folder->id == $folder->id ? 'current' : ''; ?>">
                        <?php echo anchor("admin/store/ajax/image/{$folder->id}", $folder->name, 'title="' . $folder->slug . '"'); ?>
                    </li>
                <?php endif; ?>
            <?php endforeach; ?>
            <?php if ($folders): ?>
                <li class="upload">
                    <?php echo anchor("admin/store/ajax/image/#upload", lang('files:upload'), 'title="upload" id="open-upload"'); ?>
                </li>
                <li class="folder">
                    <?php echo anchor("admin/store/ajax/image/#folder", lang('files:new_folder'), 'title="Folder" id="new-folder"'); ?>
                </li>
            <?php endif; ?>
        </ul>
    </div>
                        ...
                        <tbody>
                            <?php foreach ($current_folder->items as $image): ?>
                                <tr class="<?php echo alternator('', 'alt'); ?>">
                                    <td class="image">
                                        <img class="pyro-image" src="<?php echo base_url($image->location === 'local' ? 'files/thumb/' . $image->id . '/50/50' : 'files/cloud_thumb/' . $image->id); ?>" alt="<?php echo $image->name; ?>" width="50" onclick="javascript:window.parent.selectImage('<?php echo $image->id; ?>');" /></td>
...
<script>
    (function ($) {
        $(document).ready(function () {
            $('#open-upload').live('click', function (e) {
                $('#upload-box').fadeIn(500);
                return false;
            });
            $('#new-folder').live('click', function (e) {
                $('#directory-box').fadeIn(500);
                return false;
            });
        });
    })(jQuery)
